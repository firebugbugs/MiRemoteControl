...
{
    public static List<WindowTarget> Find(string executable)
    {
    ...
    public static WindowTarget Select(List<WindowTarget> windows, IReadOnlyDictionary<string,string> args)
    {
        ...
        throw new ControlException(windows.Count == 0 ? "WindowNotFound" : "AmbiguousWindow",
            windows.Count == 0 ? "ZCode 没有可见窗口，请执行 open。" : "存在多个目标窗口，请使用 --window 指定。");
    }
    // Window area for deterministic candidate ordering when several ZCode
    ...
    public static void Focus(WindowTarget target, bool promoteToTopmost = false)
    {
        ...
        // Windows may leave ZCode visible only as a taskbar button.
    ...
    {
        Validate(target);
        if (GetForegroundWindow() != target.Handle) throw new ControlException("TargetChanged", "焦点已离开 ZCode，停止输入。");
    }

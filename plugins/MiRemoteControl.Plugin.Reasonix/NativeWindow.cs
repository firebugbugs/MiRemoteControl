...
internal static class NativeWindow
{
    ...
    /// accepted by <paramref name="isTargetProcess"/>. The ReasoniX desktop app
    /// runs from a version-stamped directory that changes on every update, so
    /// the caller supplies the identity rule instead of a fixed exe path.
    /// </summary>
    ...
    public static WindowTarget Select(List<WindowTarget> windows, IReadOnlyDictionary<string,string> args)
    {
        ...
        throw new ControlException(windows.Count == 0 ? "WindowNotFound" : "AmbiguousWindow",
            windows.Count == 0 ? "ReasoniX 没有可见窗口，请执行 open。" : "存在多个目标窗口，请使用 --window 指定。");
    }
    // Window area for deterministic candidate ordering when several ReasoniX
    ...
    public static void Focus(WindowTarget target, bool promoteToTopmost = false)
    {
        ...
        // Windows may leave ReasoniX visible only as a taskbar button.
        ...
        try
        {
            ...
            // TV editor. Only an explicit open operation may promote ReasoniX;
        ...
        finally
        {
            ...
            // leave ReasoniX permanently above every other application.
            if (promoteToTopmost)
        ...
        if (GetForegroundWindow() != target.Handle)
        {
            ...
            if (promoteToTopmost)
                SetWindowPos(target.Handle, HWND_NOTOPMOST, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_SHOWWINDOW);
            throw new ControlException("FocusDenied", "Windows 未允许切到 ReasoniX 前台。请先激活 ReasoniX，再执行命令。");
        }
    }
    ...
    {
        Validate(target);
        if (GetForegroundWindow() != target.Handle) throw new ControlException("TargetChanged", "焦点已离开 ReasoniX，停止输入。");
    }
    ...
    public static void Text(WindowTarget target, string text, Action checkFocus)
    {
        ...
        foreach (var line in text.Split('\n').Select((value, index) => (value, index)))
        {
            ...
            // ReasoniX maps bare Enter to "send"; Shift+Enter is the app's own
            // newline combo, so a multi-line draft is never submitted midway.
            if (line.index > 0) Key(target, 0x0D, 0x10);
            foreach (var rune in line.value.EnumerateRunes())
            {
                ...
                // The React-controlled textarea must process each input event
                // before the next one or the renderer can retain only the
